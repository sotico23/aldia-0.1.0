...
class SecurityHeaders
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (app()->environment('production') && ! Vite::isRunningHot()) {
            ...
            $response->headers->set('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' {$viteOrigins}; style-src 'self' 'unsafe-inline' https://fonts.bunny.net; img-src 'self' data: https:; font-src 'self' https://fonts.bunny.net; connect-src 'self' {$viteOrigins} {$wsOrigins} https://api.telegram.org https://graph.facebook.com; frame-ancestors 'none';");
        }
